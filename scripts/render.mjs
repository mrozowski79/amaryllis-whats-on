...
function monthGrid(start, language, locale, eventsByDate) {
  ...
  for (let index = 0; index < offset + lastDay.getDate(); index += 1) {
    ...
    const eventLinks = (eventsByDate.get(dateIso) || []).map(event => '<a class="calendar-event' + (event.area === 'Wengen' ? ' nearby' : '') + '" href="#event-' + esc(event.id) + '">' + esc(event.title[language]) + '</a>').join('');
